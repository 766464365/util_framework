...
import com.xuwei.framework.popuppicker.entity.City;
import com.xuwei.framework.popuppicker.entity.County;
import com.xuwei.framework.popuppicker.entity.Province;
import com.xuwei.framework.popuppicker.util.ConvertUtils;
...
/**
 * 地址选择器（包括省级、地级、县级），地址数据见assets目录下"assets/city.json"文件。
...
public class AddressPicker extends LinkagePicker<Province, City, County> {
    ...
    public AddressPicker(Activity activity) {
        super(activity);
        ArrayList<Province> defaultProvience = readProvince(activity);
        Provider defaultProvider = new AddressProvider(defaultProvience);
        setProvider(defaultProvider);
        this.provinces = defaultProvience;
    }
    ...
    protected View makeCenterView() {
        ...
        provinceView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
            ...
            public void onSelected(int index) {
                ...
                CommonLog.v( "change cities after province wheeled: index=" + index);
        ...
        cityView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
            ...
            public void onSelected(int index) {
                ...
                CommonLog.v( "change counties after city wheeled: index=" + index);
                selectedThirdIndex = 0;//重置县级索引
                //根据地市获取区县
                //noinspection unchecked
                List<County> counties = provider.linkageThirdData(selectedFirstIndex, selectedSecondIndex);
    ...
    public void setSelectedIndex(int firstIndex, int secondIndex, int thirdIndex) {
        super.setSelectedIndex(firstIndex, secondIndex, thirdIndex);
    }

    /**
     * 从“assets/city.json”文件中读取省市区数据
     * @param activity
     * @return
     */
    private ArrayList<Province> readProvince(Activity activity) {
        String json;
        try {
            json = ConvertUtils.toString(activity.getAssets().open("city.json"));
            Gson gson = new Gson();
            Type type = new TypeToken<ArrayList<Province>>() {}.getType();
            ArrayList<Province> defaultPro = gson.fromJson(json, type);
            return defaultPro;
        } catch (IOException e) {
            CommonLog.e(e,"读取默认的省市区数据出错", "");
        }
        return null;
    }

...
import com.xuwei.framework.popuppicker.entity.LinkageFirst;
import com.xuwei.framework.popuppicker.entity.LinkageSecond;
import com.xuwei.framework.popuppicker.entity.LinkageThird;
import com.xuwei.framework.popuppicker.widget.WheelView;
...
public class LinkagePicker<Fst extends LinkageFirst<Snd>, Snd extends LinkageSecond<Trd>, Trd>
     ...
     */
    @Deprecated
    public LinkagePicker(Activity activity, List<Fst> f, List<List<Snd>> s, List<List<List<Trd>>>
        t) {
        super(activity);
    ...
    public void setSelectedItem(Fst fst, Snd snd, Trd trd) {
        ...
        CommonLog.v("init select first: " + fst.getName() + ", index:" + selectedFirstIndex);
        //noinspection unchecked
        ...
        CommonLog.v("init select second: " + snd.getName() + ", index:" + selectedSecondIndex);
        ...
        CommonLog.v("init select third: " + trd + ", index:" + selectedThirdIndex);
    }
    ...
    public Snd getSelectedSecondItem() {
        if (selectedSecondItem == null) {
            //noinspection unchecked
            selectedSecondItem = (Snd) provider.linkageSecondData(selectedFirstIndex).get
                (selectedSecondIndex);
        }
    ...
    public Trd getSelectedThirdItem() {
        if (selectedThirdItem == null) {
            List<Trd> thirdData = provider.linkageThirdData(selectedFirstIndex,
                selectedSecondIndex);
            if (thirdData.size() > 0) {
    ...
    protected View makeCenterView() {
        ...
        firstView.setLayoutParams(new LinearLayout.LayoutParams(0, WRAP_CONTENT,
            firstColumnWeight));
        ...
        secondView.setLayoutParams(new LinearLayout.LayoutParams(0, WRAP_CONTENT,
            secondColumnWeight));
        ...
        if (!provider.isOnlyTwo()) {
            thirdView.setLayoutParams(new LinearLayout.LayoutParams(0, WRAP_CONTENT,
                thirdColumnWeight));
        ...
        firstView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
            ...
            public void onSelected(int index) {
                ...
                CommonLog.v( "change second data after first wheeled");
                ...
                if (!provider.isOnlyTwo()) {
                    //根据第二级数据获取第三级数据
                    //noinspection unchecked
                    List<Trd> trds = provider.linkageThirdData(selectedFirstIndex,
                        selectedSecondIndex);
        ...
        secondView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
            ...
            public void onSelected(int index) {
                ...
                if (!provider.isOnlyTwo()) {
                    CommonLog.v( "change third data after second wheeled");
                    selectedThirdIndex = 0;//重置第三级索引
                    //noinspection unchecked
                    List<Trd> trds = provider.linkageThirdData(selectedFirstIndex,
                        selectedSecondIndex);
        ...
        thirdView.setItems(provider.linkageThirdData(selectedFirstIndex, selectedSecondIndex),
            selectedThirdIndex);
        thirdView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
            @Override
            public void onSelected(int index) {
                //noinspection unchecked
                selectedThirdItem = (Trd) provider.linkageThirdData(selectedFirstIndex,
                    selectedSecondIndex).get(index);
                selectedThirdIndex = index;
                if (onWheelLinkageListener != null) {
                    onWheelLinkageListener.onLinkage(selectedFirstIndex, selectedSecondIndex,
                        selectedThirdIndex);
                }
                if (onWheelListener != null) {
                    String thirdName;
     ...
     * 数据选择完成监听器
     */
    public static abstract class OnStringPickListener implements
        OnPickListener<StringLinkageFirst, StringLinkageSecond, String> {
     ...
     * 兼容旧版API
     */
    public static abstract class DataProvider implements Provider<StringLinkageFirst,
        StringLinkageSecond, String> {
     ...
     * 默认的数据提供者
     */
    private static class DefaultDataProvider<Fst extends LinkageFirst<Snd>, Snd extends
        LinkageSecond<Trd>, Trd> implements Provider<Fst, Snd, Trd> {
